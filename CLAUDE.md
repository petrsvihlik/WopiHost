...
- **Rename is editor-driven, never host-page-driven.** The WOPI client owns the rename affordance (its editable title, surfaced by `SupportsRename` + the `UserCanRename` token flag scoped to edit sessions); the editor calls the backend's `RENAME_FILE` op, which renames under the same file id and keeps the live session working. A host-page rename button is the wrong shape: there's no host→editor "start rename" postMessage, a direct backend `RENAME_FILE` from the frontend collides with the editor's edit lock (409), and the frontend's per-process id↔path map would go stale (the failure that reverted the toolbar-rename attempt). The host page only *reflects* the result — Office posts `File_Rename` (`Values.NewName`) which `WopiLayout.razor` folds into the toolbar label; Collabora renames via its own title bar but doesn't post it (label refreshes on next load); ONLYOFFICE may not surface rename at all.
