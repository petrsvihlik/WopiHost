...
| `AppHost:OnlyOfficeProofValidation` | **Default: `false`.** ONLYOFFICE advertises a `<proof-key>` in its discovery, but in practice its WOPI `GetFile` callback is **rejected** by the host's `WopiProofValidator` (the editor shell loads, then the document download 500s and ONLYOFFICE shows "Download failed"). So the lane defaults to proof **off** to be functional out of the box, same as Collabora. Set this to `true` to drive the real `WopiProofValidator` path against ONLYOFFICE — useful for investigating why its signature is rejected (does it send `X-WOPI-Proof` at all?), but expect "Download failed" until that's resolved. No effect unless `AppHost:UseOnlyOffice=true`. |
